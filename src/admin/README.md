...
| ------ | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 200    | نجاح                  | `{ "message": "تم تحديث المحتوي بنجاح", "content": { "_id": "64f1e3d9a1b23d0012c34567", "title": "عنوان جديد", "description": "وصف محدث" } }` |
| 404    | المحتوى غير موجود     | `{ "message": "حدث خطأ غير متوقع يرجي اعادة المحاوله " }`                                                                                                          |
| 400    | خطأ تحقق (Validation) | `{ "message": "رجاك ارسال البانات المطلوب تعديلها" }`                                                                                                              |

---

## ✏️ حذف المحتوى (DELETE /api/admin/content-Delete/\:id)

| Status | Description                  | Response Example                                                      |
| ------ | ---------------------------- | --------------------------------------------------------------------- |
| 200    | نجاح                         | `{ "message": "تم الحذف بجاح" }`                                      |
| 404    | المحتوى غير موجود أو تم حذفه | `{ "message": "المحتوى غير موجود أو تم حذفه بالفعل" }`                |
| 400    | خطأ تحقق (ID غير موجود)      | `{ "message": "رجاك ارسال ال ID الخاص بالمحتوي " }`                   |
| 500    | خطأ سيرفر                    | `{ "error": "خطأ في حذف المحتوى", "details": "رسالة الخطأ الفعلية" }` |
...
PATCH /api/admin/EditEvents/\:id

````

### الوصف  
يتم تعديل حدث موجود باستخدام الـ **ID** الخاص به.  

### البارامترات (Params)
- `id` : معرف (ID) الحدث المطلوب تعديله.

### الطلب (Request Body مثال)
```json
{
  "title": "تعديل عنوان الفعالية",
  "type": "تعليم",
  "date": "2025-09-10",
  "time": "14:30",
  "location": {
...
| الكود   | الرسالة / الاستجابة                                                                                                        |
| ------- | -------------------------------------------------------------------------------------------------------------------------- |
| **200** | `json { "message": "تم تحديث المحتوي بنجاح", "Event": { ... } } `                                                          |
| **400** | `json { "message": "رجاك ارسال ال ID الخاص بالمحتوي " } ` <br> `json { "message": "رجاك ارسال البانات المطلوب تعديلها" } ` |
| **404** | `json { "message": "خطا غير متوقع اثناء التعديل" } `                                                                       |
| **500** | `json { "error": "خطا في تعديل الحدث", "details": "Cast to ObjectId failed ..." } `                                        |

---

## 🗑️ حذف حدث (Delete Event)

**Endpoint**

```
DELETE /api/admin/DeleteEvent/:id
```

### الوصف

يتم حذف حدث باستخدام الـ **ID** الخاص به.

### البارامترات (Params)

* `id` : معرف (ID) الحدث المطلوب حذفه.

### الاستجابات (Responses)

| الكود   | الرسالة / الاستجابة                                                                              |
| ------- | ------------------------------------------------------------------------------------------------ |
| **200** | `json { "message": "تم الحذف بجاح" } `                                                           |
| **400** | `json { "message": "رجاك ارسال ال ID الخاص بالمحتوي " } `                                        |
| **404** | `json { "message": "الفاعليه غير موجوده أو تم حذفه بالفعل" } `                                   |
| **500** | `json { "error": "خطا في حذقف الفاعليت او الاحداث", "details": "Cast to ObjectId failed ..." } ` |

---

## 🔑 الملاحظات الخاصة بالمصادقة (Authentication)

* يجب إرسال **الكوكيز** مع الطلب (`credentials: "include"` أو `withCredentials: true`).
...
```js
// تعديل حدث
fetch('/api/admin/EditEvents/64acde98f1234567890abcd', {
  method: 'PATCH',
  headers: {
    'Content-Type': 'application/json'
  },
  credentials: 'include', // ارسال الكوكيز
  body: JSON.stringify({
    title: 'عنوان جديد',
    type: 'رياضه',
    date: '2025-09-12',
    time: '18:00',
    location: { type: 'Point', coordinates: [31.22, 30.05] }
  })
})
  .then(res => res.json())
  .then(data => console.log(data));
...
// حذف حدث (بسيط)
fetch('/api/admin/DeleteEvent/64acde98f1234567890abcd', {
  method: 'DELETE',
  credentials: 'include' // ارسال الكوكيز
})
  .then(res => res.json())
...
// تعديل حدث
axios.patch(
  '/api/admin/EditEvents/64acde98f1234567890abcd',
  {
    title: 'عنوان جديد',
    type: 'رياضه',
    date: '2025-09-12',
  ...
  { withCredentials: true } // ارسال الكوكيز
)
.then(res => console.log(res.data))
.catch(err => console.error(err));
...
// حذف حدث (بسيط)
axios.delete(
  '/api/admin/DeleteEvent/64acde98f1234567890abcd',
  { withCredentials: true } // ارسال الكوكيز
)
.then(res => console.log(res.data))
.catch(err => console.error(err));
